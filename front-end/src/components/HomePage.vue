<template>
<header class="flex justify-center items-center fixed z-10 w-full h-10 sm:h-14 overflow-hidden">
  <h1 class="font-serif font-bold text-2xl text-white">Wrapped</h1>
</header>
<div>
  <div class = "w-screen h-screen bg-gradient-to-b from-gray-100 overflow-y-auto snap-y snap-mandatory">
    <div v-for="country in wrapped"> <div v-for="trend, key in country"> <div v-for="news, key in trend">
          <div class="snap-center relative flex flex-col justify-center h-screen">
            <ul v-for="ne, key in news" class="flex flex-col justify-center sm:y-14" id="zeta">
              <div class="tip bottom px-3 sm:pt-1"><h2 class="font-serif tracking-wide text-gray-700 sm:text-2xl text-xl">{{key}}</h2></div>
              <div class="flex sm:justify-center overflow-x-auto gap-6 snap-x snap-mandatory sm:snap-normal before:shrink-0 before:w-[20%] sm:before:w-[0%] after:shrink-0 after:w-[20%] sm:after:w-[0%] sm:pt-7 sm:pb-12 pb-16">
...
header {
  ...
  box-shadow: 0 1em 0.5em -0.5em rgba(126, 184, 124, 0.424);
}
