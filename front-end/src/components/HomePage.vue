<template>
<header class="flex justify-center items-center relative z-10 w-full overflow-hidden drop-shadow-md">
  <h1 class="font-serif font-bold text-2xl text-white">Wrapped</h1>
</header>
  <div class = "w-full alt-93 bg-gradient-to-b from-gray-100 overflow-y-auto snap-y snap-mandatory">
    <div v-for="country in wrapped"> <div v-for="trend, key in country"> <div v-for="news, key in trend">
          <div class="snap-center relative flex flex-col justify-center">
            <ul v-for="ne, key in news" class="flex flex-col alt-93 justify-center sm:y-14 sm:pt-7">
              <div class="tip bottom px-2 pb-1 drop-shadow-md"><h2 class="font-serif tracking-wide text-gray-700 text-xl">{{key}}</h2></div>
              <div class="flex sm:justify-center overflow-x-auto gap-6 snap-x snap-mandatory sm:snap-normal before:shrink-0 before:w-[20%] sm:before:w-[0%] after:shrink-0 after:w-[20%] sm:after:w-[0%] pt-7 pb-14">
